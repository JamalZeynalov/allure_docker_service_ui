...
def generate_tests_history_plot(daily_reports: List[str]):
    ...
    plotdata = pd.DataFrame(
        {"failed": failed, "broken": broken, "skipped": skipped, "passed": passed,},
        index=indexes,
    )
    ...
    plt.title(
        "Daily History", fontdict={"family": "serif", "weight": "normal", "size": 16}
    )
    plt.ylabel("Executed test cases count", fontdict={"weight": "normal", "size": 12})
